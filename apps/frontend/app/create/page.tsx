...
export default function CreateChallenge() {
  ...
  async function improveChallenge(instruction = "Improve clarity and make the MVP more actionable.") {
    if (!challenge || !analysis) return;
    setLoading(true); setError("");
    try {
      const response = await fetch(`${API_URL}/api/improve`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ description, challenge, instruction }) });
      if (!response.ok) throw new Error("The challenge could not be improved.");
      const result = await response.json();
      setChallenge(result.challenge);
      setAnalysis({ ...analysis, ...result });
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to improve the challenge."); } finally { setLoading(false); }
  }

  async function translateChallenge(targetLanguage: "ru" | "kk" | "en") {
    if (!challenge || !analysis) return;
    setLoading(true); setError("");
    try {
      const response = await fetch(`${API_URL}/api/translate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ challenge, target_language: targetLanguage }) });
      if (!response.ok) throw new Error("The challenge could not be translated.");
      const result = await response.json();
      setChallenge(result.challenge);
      setAnalysis({ ...analysis, ...result });
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to translate the challenge."); } finally { setLoading(false); }
  }

  async function regenerateQuestions() {
    setLoading(true); setError("");
    try {
      const response = await fetch(`${API_URL}/api/questions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ description, answers }) });
      if (!response.ok) throw new Error("Questions could not be regenerated.");
      const result = await response.json();
      setAnalysis({ ...(analysis || {}), ...result });
      setAnswers({});
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to regenerate questions."); } finally { setLoading(false); }
  }
  ...
  return <main className="min-h-screen bg-[#f7f8fc]">
    ...
    <div className="mx-auto max-w-4xl px-6 py-16"><Link href="/" className="mb-10 inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-black"><ArrowLeft size={16} /> Back</Link>
      {!analysis ? <InputStep description={description} setDescription={setDescription} onAnalyze={analyzeProblem} loading={loading} /> : !challenge ? <AnalysisStep analysis={analysis} answers={answers} setAnswers={setAnswers} onGenerate={generateChallenge} onRegenerate={regenerateQuestions} loading={loading} /> : <ChallengeStep challenge={challenge} score={analysis.score} provider={analysis.provider} model={analysis.model} usage={analysis.usage} loading={loading} onImprove={improveChallenge} onTranslate={translateChallenge} />}
